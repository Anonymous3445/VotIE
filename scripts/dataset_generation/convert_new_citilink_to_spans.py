#!/usr/bin/env python3
"""
Convert New CitiLink Dataset to Span-Annotated JSONL
...
Entity types (14): SUBJECT, SUB-SUBJECT, VOTING, VOTER-FAVOR, VOTER-AGAINST,
                   VOTER-ABSTENTION, VOTER-ABSENT, COUNTING-UNANIMITY, COUNTING-MAJORITY,
                   COUNT-FAVOR, COUNT-AGAINST, COUNT-ABSTENTION, COUNT-BLANK, VOTING-METHOD

Usage:
    python scripts/dataset_generation/convert_new_citilink_to_spans.py
    python scripts/dataset_generation/convert_new_citilink_to_spans.py --input data/citilink-dataset/ --output-dir data/citilink_spans
"""
...
def convert_subject_to_span_example(
    ...
    # When voting_evidence is empty, the event is associated with the
    # previous one (same event_id) per annotator convention.
    voting_events = subject.get("voting", [])
    event_id = 0
    for voting in voting_events:
        ve = voting.get("voting_evidence")
        has_evidence = ve and ve.get("text")
        if has_evidence:
            event_id += 1
        elif event_id == 0:
            # First event has no evidence — still assign event_id 1
            event_id = 1
        # else: reuse previous event_id
        event_spans, event_warnings = convert_voting_event(
            voting, subject_start, event_id
        )
        spans.extend(event_spans)
        warnings.extend(event_warnings)
    num_distinct_events = event_id

    # Process sub_subjects → SUB-SUBJECT spans + any nested voting events
    next_event_id = num_distinct_events + 1
    for ss in subject.get("sub_subjects", []):
        if not ss.get("text"):
            continue

        # Assign event_id: find the closest subject-level voting event by
        # position, or use the next available event_id for sub_subjects that
        # carry their own voting arrays.
        ss_voting = ss.get("voting", [])

        if ss_voting:
            # Sub_subject has its own voting events (e.g., Porto secret ballots).
            # Each gets a fresh event_id; the SUB-SUBJECT span shares the first.
            first_event_id = next_event_id
            spans.append(make_span(
                ss["text"], "SUB-SUBJECT",
                ss["start"], ss["end"],
                first_event_id, subject_start,
            ))
            for ss_vote in ss_voting:
                ss_spans, ss_warns = convert_voting_event(
                    ss_vote, subject_start, next_event_id,
                )
                spans.extend(ss_spans)
                warnings.extend(ss_warns)
                next_event_id += 1
        else:
            # No own voting — assign to nearest subject-level voting event
            # by proximity (closest voting_evidence start position).
            ss_mid = (ss["start"] + ss["end"]) / 2
            best_event_id = 1  # fallback
            best_dist = float("inf")
            for idx, voting in enumerate(voting_events):
                ve = voting.get("voting_evidence")
                if ve and ve.get("start") is not None:
                    dist = abs(ve["start"] - ss_mid)
                    if dist < best_dist:
                        best_dist = dist
                        best_event_id = idx + 1
            spans.append(make_span(
...
def validate_completeness(
    ...
    """
    Cross-validate converted output against the original dataset to ensure
    no sub_subject or voting information was lost.

    Checks:
    1. Every sub_subject text appears as a SUB-SUBJECT span.
    2. Every voting event (including those nested inside sub_subjects) has
       a corresponding VOTING span.
    3. Total voting event counts match between source and converted output.
    """
    errors = []

    # Build lookup: subject_id → converted example
    all_examples = {}
    ...
    total_sub_subject_voting = 0
    matched_sub_subjects = 0
    matched_sub_subject_voting = 0

    for municipality_data in all_municipalities:
        for minute in municipality_data["minutes"]:
            minute_id = minute["minute_id"]
            if get_split_for_minute(minute_id, splits) is None:
                continue

            for agenda_item in minute.get("agenda_items", []):
                for subject in agenda_item.get("subjects", []):
                    subject_id = subject["subject_id"]
                    ex = all_examples.get(subject_id)
                    if ex is None:
                        continue

                    subject_start = subject["start"]
                    sub_subject_spans = [
                        s for s in ex["spans"] if s["label"] == "SUB-SUBJECT"
                    ]
                    voting_spans = [
                        s for s in ex["spans"] if s["label"] == "VOTING"
                    ]
                    ...
                    for ss in subject.get("sub_subjects", []):
                        if not ss.get("text"):
                            continue
                        total_sub_subjects += 1
                        expected_start = ss["start"] - subject_start
                        expected_end = ss["end"] - subject_start
                        found = any(
                            s["start"] == expected_start
                            and s["end"] == expected_end
                            and s["text"] == ss["text"]
                            for s in sub_subject_spans
                        )
                        if found:
                            matched_sub_subjects += 1
                        else:
                            errors.append(
                                f"{subject_id}: missing SUB-SUBJECT "
                                f"'{ss['text'][:60]}' at [{expected_start}:{expected_end}]"
                            )

                        # Check voting events nested inside sub_subjects
                        for ss_vote in ss.get("voting", []):
                            ve = ss_vote.get("voting_evidence")
                            if not ve or not ve.get("text"):
                                continue
                            total_sub_subject_voting += 1
                            expected_vs = ve["start"] - subject_start
                            expected_ve = ve["end"] - subject_start
                            found_v = any(
                                s["start"] == expected_vs
                                and s["end"] == expected_ve
                                for s in voting_spans
                            )
                            if found_v:
                                matched_sub_subject_voting += 1
                            else:
                                errors.append(
                                    f"{subject_id}: missing VOTING from sub_subject "
                                    f"'{ve['text']}' at [{expected_vs}:{expected_ve}]"
                                )

                    # Check total voting count (subject-level + sub_subject-level).
                    # Events with empty voting_evidence share the previous
                    # event's VOTING span, so only count non-empty ones.
                    expected_voting_count = sum(
                        1 for v in subject.get("voting", [])
                        if v.get("voting_evidence", {}).get("text")
                    )
                    for ss in subject.get("sub_subjects", []):
                        expected_voting_count += sum(
                            1 for v in ss.get("voting", [])
                            if v.get("voting_evidence", {}).get("text")
                        )
                    if len(voting_spans) != expected_voting_count:
                        errors.append(
                            f"{subject_id}: voting count mismatch: "
                            f"expected {expected_voting_count}, got {len(voting_spans)}"
                        )

    logger.info(
        f"Completeness check: "
        f"sub_subjects={matched_sub_subjects}/{total_sub_subjects}, "
        f"sub_subject_voting={matched_sub_subject_voting}/{total_sub_subject_voting}"
    )

    if errors:
        logger.error(f"Completeness validation failed: {len(errors)} errors")
        for e in errors[:20]:
            logger.error(f"  {e}")
        if len(errors) > 20:
            logger.error(f"  ... and {len(errors) - 20} more")
    else:
        logger.info("Completeness validation passed: no information lost")
...
def main():
    ...
    logger.info("")
    logger.info("Validating completeness against source data...")
    complete, completeness_errors = validate_completeness(
        args.input, examples, splits
    )
